...
code(r'''# 우리가 결과에 쓸 수 있는 문장은 어느 것인가 — 정답과 이유를 함께 읽는다
statements = {
...
code(r'''# ▶ feature(X) 와 target(y) 의 분류 — 🖐 실행 전에 각자 답해 보게 한 뒤 셀을 돌린다
items = {
    "2015년 가족지지 점수":        "X",
...
code(r'''# 데모 인벤토리에서 '두 해 모두 존재하는 ID' 가 몇 명인지 세어 본다
import pandas as pd
...
code(r'''# ▶ 서술형 과제 — 여기만은 '내 문장'으로 쓴다 (2차시 시작 때 발표)
